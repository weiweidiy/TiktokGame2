    ...
    internal class UIParticleRenderer : MaskableGraphic
    {
        ...
        public void UpdateMesh(Camera bakeCamera)
        {
            ...
            if (_isTrail && _parent.canSimulate && 0 < _particleSystem.particleCount)
            {
                ...
                {
                    //Issues
                    //I get an error when I open a prefab with particle effects and then close UnityWhen I opened the prefab with particle effects and then closed Unity, an error was triggered
            ...
            else if (!_isTrail && _renderer.CanBakeMesh())
            {
                ...
                {
                    //Issues
                    //I get an error when I open a prefab with particle effects and then close Unity
                    //Assertion failed on expression: 'ps->array_size()'
                    //Mesh '[UIParticleRenderer] Combine Instance Mesh': abnormal mesh bounds - most likely it has some invalid vertices (+/-inifinity or NANs) due to errors exporting.
                }
#endif
